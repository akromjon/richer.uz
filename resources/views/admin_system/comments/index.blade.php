...
<div class="container-fluid">
    <div class="row">
                    ...
                    <table class="table table-responsive table-hover bg-light">
                        ...
                        <tbody>
                            <tr>
                                ...
                                <td class="txt-oflo">{{$course->name}}<input id="course" type="hidden" value="{{$comment->course_id}}" ></td>
                                <td><a style="display: inline-block; width:60px;" href="{{route('edit_course_comment',$comment->id)}}" class="label btn btn-success label-rounded">edit</a>
                                    <form style="display: inline-block; width:60px;" method="post" action="{{route('delete_course_comment',$comment->id)}}">
                                        @csrf
                                        {{ method_field('DELETE') }}
                                ...
                                <td class="txt-oflo">
                                    <form class="form-group " action="{{route('update_confirmation',$comment->id)}}" method="POST" enctype="multipart/form-data">
                                        @csrf
                                        {{ method_field('PUT') }}
        ...
        <div class="col-6">
            <div class="card ">
                <div class="card-body">
                    <form action="{{route('reply_comment')}}" method="POST" enctype="multipart/form-data">
                        @csrf
                        <div class="form-group">
